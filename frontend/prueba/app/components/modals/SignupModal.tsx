    ...
    const content = (
        <>
            <form 
                ...
                className="space-y-4"
            >
                <input onChange={(e) => setUsername(e.target.value)} placeholder="Your username" type="username" className="w-full h-[54px] px-4 border border-gray-300 rounded-xl" />
                ...
                {errors.map((error, index) => {
                    return (
                        <div 
                            key={`error_${index}`}
                            className="p-5 bg-excuela text-white rounded-xl opacity-80"
                        >
                            {error}
                        </div>
                    )
